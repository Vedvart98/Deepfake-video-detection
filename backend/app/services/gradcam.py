...
class GradCAMExplainer:
    ...
    def _setup_cam(self):
        ...
        try:
            ...
            if "vit" in self.target_layer_name:
    ...
    def _get_target_layers(self) -> List[torch.nn.Module]:
        ...
        if not layers and hasattr(self.model, "cnn"):
            cnn_features = None
            for name, module in self.model.named_modules():
                if name == "cnn.features":
                    cnn_features = module
                    break
            if cnn_features is not None:
                layer4 = None
                for name, module in cnn_features.named_children():
                    if name == "7" or "layer4" in str(type(module).__name__).lower():
                        layer4 = module
                        break
                if layer4 is not None:
                    last_conv = None
                    for _, module in layer4.named_modules():
